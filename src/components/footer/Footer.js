...
function Footer() {
  const SocialButton = ({ iconPath, onClick, label }) => (
    <button
      className="w-[25%] flex justify-center items-center p-2 hover:bg-gray-300 rounded"
      onClick={onClick}
      aria-label={label}
    >
      <svg
  ...
  const buttons = [
    ...
    },
    {
      ...
      onClick: () => alert("붕어빵 탐험대 Instagram 준비 중"),
    },
  ];
  ...
  return (
    <div>
      <div className="mx-auto max-h-[90px] bg-gray-200">
        <div className="w-full text-sz30 flex bg-white text-point-color">
          {buttons.map((btn, idx) => (
            <SocialButton
              key={idx}
